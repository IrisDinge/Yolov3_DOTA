...
class Darknet53spp(nn.Module):
    custom_layers = (bdkn.Stage, bdkn.HeadBody, bdkn.Transition,
                     ...
                     bdkn.SPPBody, bdkn.DCNv2, bdkn.DCNv2)
    ...
    def __init__(self):
        ...
        layer_list = [
            ...
            OrderedDict([
                ('stage_1', vn_layer.Conv2dBatchLeaky(3, input_channels, 3, 1, 1)),     # DBL
                ('stage_2', bdkn.Stage(input_channels, stage_cfg['stage_2'])),          # res1
                ('stage_3', bdkn.Stage(input_channels*(2**1), stage_cfg['stage_3'])),   # res2
                ('stage_4', bdkn.DCNv2(input_channels*(2**2), stage_cfg['stage_4'])),   # res8
            ]),

            # layer 1
            # second scale
            OrderedDict([
                ('stage_5', bdkn.Stage(input_channels*(2**3), stage_cfg['stage_5'])),   # res
            ]),

            # layer 2
            # third scale, largest
            OrderedDict([
                ('stage_6', bdkn.Stage(input_channels*(2**4), stage_cfg['stage_6'])),   # res
            ]),
            ...
            OrderedDict([
                ('head_body_1', bdkn.HeadBody(input_channels*(2**5), first_head=True)),  # 5DBL
            ]),
            ...
            OrderedDict([
                ('head_body_3', bdkn.SPPBody(input_channels*(2**3+2**2))),             #5DBL: (1-3)DBL + SPP +(4-5)
            ]),
